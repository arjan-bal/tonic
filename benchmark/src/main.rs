...
use benchmark::worker::{
    client_args, server_args, worker_service_server::WorkerService,
            ...
            while let Some(request) = stream.next().await {
                ...
                match  argtype {
                    client_args::Argtype::Setup(client_config) => {
                        println!("Client creation requested.");
                        if let Some(mut client) = benchmark_client.take() {
                    ...
                    client_args::Argtype::Mark(mark) => {
                        println!("Client stats requested.");
